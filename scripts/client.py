    ...
    while True:
        ...
        match phase:
            ...
            case 'ChampSelect':
                handle_game_lobby()
            case 'InProgress':
                if SELECTED_LANE == 'mid':
                    game.play_game()
                elif SELECTED_LANE == 'jungle':
                    junglegame.play_game()
            case 'Reconnect':
                reconnect()
            ...
            if not action['completed']:
                ...
                if action['championId'] == 0:  # no champ selected, attempt to select a champ
                    ...
                    if SELECTED_LANE == 'mid':
                        ...
                        if champ_index < len(JUNGCHAMPS):
                            champion_id = JUNGCHAMPS[champ_index]
                            champ_index += 1
                        else:
                            champion_id = f2p[f2p_index]
                            f2p_index += 1
                    ...
                    if not requested:
                        if SELECTED_LANE == 'mid':
                            ...
                            sleep(1)
                            chat(random.choice(ASK_4_JUNG_DIALOG), 'handle_game_lobby')
                            requested = True
            else:
...
def get_cs():
    ...
    r_json = r.json()  # convert the response to JSON
    summoner_name = r_json['activePlayer']['summonerName']

    # Find active player in allPlayers array
    for player in r_json['allPlayers']:
        if player['summonerName'] == summoner_name:
            creep_score = player['scores']['creepScore']
            return creep_score

    # In case the active player is not found in the allPlayers array
    print("Could not get creep score")
    return None
...
def get_id():
    ...
    for i in range(3):
        if r.status_code == 200:
            champion_id = r['lol']['championId']
            return champion_id
        sleep(1)
    log.warning("Could not get champion id")
