    ...
    while True:
        ...
        try:
            ...
            # Game Start
                ...
                utils.press('ctrl+q')
                utils.attack_move_click(GAME_JUNGLE_CAMP_REDBUFF)
                in_lane = True
                sleep(5)
            ...
            # Game Running
                ...
                if game_time < EARLY_GAME_END_TIME:
                        if client.get_cs() < (jungCount + 4):
                            utils.attack_move_click(GAME_CENTER_OF_SCREEN, 0.5)
                            utils.attack_move_click(GAME_JUNGLE_CAMP_REDBUFF)
                            sleep(4)
                        elif (jungCount + 4) <= client.get_cs() < (jungCount + 8):
                            utils.attack_move_click(GAME_CENTER_OF_SCREEN, 0.5)
                            utils.attack_move_click(GAME_JUNGLE_CAMP_RAPTORS)
                            sleep(4)
                        elif (jungCount + 8) <= client.get_cs() < (jungCount + 12):
                            utils.attack_move_click(GAME_CENTER_OF_SCREEN, 0.5)
                            utils.attack_move_click(GAME_JUNGLE_CAMP_MURKWOLF)
                            sleep(4)
                        elif (jungCount + 12) <= client.get_cs() < (jungCount + 16):
                            utils.attack_move_click(GAME_CENTER_OF_SCREEN, 0.5)
                            utils.attack_move_click(GAME_JUNGLE_CAMP_REDBUFF)
                            sleep(4)
                        elif (jungCount + 16) <= client.get_cs() < (jungCount + 20):
                            utils.attack_move_click(GAME_CENTER_OF_SCREEN, 0.5)
                            utils.attack_move_click(GAME_JUNGLE_CAMP_GROMP)
                            sleep(4)
                        elif (jungCount + 16) <= client.get_cs() < (jungCount + 20):
                            jungCount += 20
